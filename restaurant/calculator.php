           ...
           if ($query_execute_run) {
             ...
             <tr>
              ...
              <h4>Total cost</h4>
               ...
               <button class='btn btn-warning' id='order_button' name='order_button'><font color='white'>Order</font></button>
               </div>
               </div></br>
               <center><button class='btn btn-success' id='pay_button'>Pay</button></center>
               </br>
...
<?php
          ...
          //This is the query tha handles a request executed when the button 'order' is clicked which is a request to make an order for the items the customer has selected

           if (isset($_GET['order_button'])) {

            require_once 'conn.php';
            $email = "briano@gmail.com";
            $place_order = "SELECT * FROM items_selected WHERE email = '$email' AND payment_status ='Pending'";
            $execute_query = mysqli_query($conn,$place_order);
            while ($row = mysqli_fetch_array($execute_query)) { 
              ...
              $order = "Item: ".$item." Price: ".$price." Quantity: ".$quantity." = ".$total;
              
               $insert_order = "INSERT INTO orders(email,items,total_price,payment_status) VALUES('$email','$order','$gross','$payment')";
              $execute = mysqli_query($conn,$insert_order);
              if ($execute) {
               echo "<script>alert('You have successfully placed an order')</script>";
              }
              else{
                echo "<script>alert('An error occurred while processing your order. Pleas use relevant details or try again later.')</script>";
              }
