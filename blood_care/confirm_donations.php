  ...
  <div class="container-fluid" style="overflow: scroll; height: 910px; max-height: 910px;">
    <table border="1px" width="100%" cellpadding="1px" cellspacing="1px">
      <tr>
        ...
        <th><center>Cick to confirm donation delivery</center></th>
      </tr>
<?php

              require 'connect.php';
                  
                  $sqli = "SELECT * FROM blood_donations WHERE status = 'Pending'";
                   ...
                   while($row=mysqli_fetch_assoc($result)) {
                   	$donation_id = $row['id'];
                             ...
                             echo "<td><center>".$row['status']."</center></td>";
                              echo "<td><center><form action='confirm_donations.php?donation_id=$donation_id' method='POST'><button class='btn' name='confirm'><font color='orange'>Match with recepient</font></button></form></center></td>";
                             echo "</tr>";
...
<?php
             ...
             if (isset($_POST['confirm'])) {
             	$Status_report = "Donation delivered";
             	$confirm_donation = "UPDATE blood_donations SET status = '$Status_report' WHERE id = '$donation_id'";
             	$execute_query = mysqli_query($conn,$confirm_donation);
             	if ($execute_query) {
             		echo"<script>window.alert('You have successfully confirmed this donation.')</script>";
             	}
             	else{
